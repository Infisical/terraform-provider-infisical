...
- `basic_constraints` (Block, Optional) Basic constraints for the certificate policy, controlling CA certificate issuance and path length (see [below for nested schema](#nestedblock--basic_constraints))
...
- `is_ca` (String) Whether CA certificate issuance is allowed, required, or denied. Possible values: allowed, required, denied
- `max_path_length` (Number) Maximum path length for CA certificates. Use -1 for unlimited, 0 or positive integers for specific limits.
...
- `type` (String) The subject attribute type. Possible values: common_name, organization, country, state, locality, organizational_unit
