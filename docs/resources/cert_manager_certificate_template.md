...
page_title: "infisical_cert_manager_certificate_template Resource - terraform-provider-infisical"
subcategory: ""
description: |-
  Create and manage certificate templates in Infisical. Only Machine Identity authentication is supported for this resource.
---

# infisical_cert_manager_certificate_template (Resource)
...
- `algorithms` (Block, Optional) Algorithm constraints for the certificate template. At least one signature algorithm and one key algorithm must be specified. (see [below for nested schema](#nestedblock--algorithms))
- `description` (String) The description of the certificate template
- `extended_key_usages` (Block, Optional) Extended key usage policies for the certificate template (see [below for nested schema](#nestedblock--extended_key_usages))
- `key_usages` (Block, Optional) Key usage policies for the certificate template (see [below for nested schema](#nestedblock--key_usages))
- `sans` (Block List) Subject alternative name (SAN) policies for the certificate template (see [below for nested schema](#nestedblock--sans))
- `subject` (Block List) Subject attribute policies for the certificate template (see [below for nested schema](#nestedblock--subject))
- `validity` (Block, Optional) Validity constraints for the certificate template (see [below for nested schema](#nestedblock--validity))

### Read-Only

- `id` (String) The ID of the certificate template
